...
type Copy = {
  ...
  operatorTitle: string; operatorBody: string; servicesTitle: string; servicesBody: string;
...
const copy: Record<Locale, Copy> = {
  en: {
    ...
    operatorTitle: 'Business owner', operatorBody: 'Yasaflow is an independent SaaS platform created and operated by Ramazan Berber as an individual freelancer based in Norway.', servicesTitle: 'Services', servicesBody: 'Cloud-based software for organizations, including member management, activities, communication and administration.',
  ...
  nb: {
    ...
    operatorTitle: 'Eier og ansvarlig', operatorBody: 'Yasaflow er en uavhengig SaaS-plattform som er utviklet og drives av Ramazan Berber som individuell frilanser i Norge.', servicesTitle: 'Tjenester', servicesBody: 'Skybasert programvare for organisasjoner, med medlemsadministrasjon, aktiviteter, kommunikasjon og drift samlet på ett sted.',
  ...
  tr: {
    ...
    operatorTitle: 'İşletme sahibi', operatorBody: 'Yasaflow, Norveç’te bireysel serbest çalışan Ramazan Berber tarafından geliştirilen ve işletilen bağımsız bir SaaS platformudur.', servicesTitle: 'Hizmetler', servicesBody: 'Kurumlar için üye yönetimi, etkinlikler, iletişim ve idari süreçleri bir araya getiren bulut tabanlı yazılım.',
...
export default function PublicSupportPage({ type, locale }: { type: PageType; locale: Locale }) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-950">
    <main id="main-content" className="mx-auto max-w-3xl px-5 py-16 sm:py-24">
      ...
      {type === 'contact' && <section className="mt-10 rounded-[2rem] border border-slate-200 bg-white p-7 shadow-sm sm:p-10"><Mail className="text-[#2185DC]" /><h1 className="mt-5 text-4xl font-semibold tracking-[-.04em]">{t.contactTitle}</h1><p className="mt-5 text-lg leading-8 text-slate-600">{t.contactIntro}</p><div className="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-5 text-left"><h2 className="text-sm font-semibold uppercase tracking-[.16em] text-[#2185DC]">{t.operatorTitle}</h2><p className="mt-3 leading-7 text-slate-700">{t.operatorBody}</p><h2 className="mt-6 text-sm font-semibold uppercase tracking-[.16em] text-[#2185DC]">{t.servicesTitle}</h2><p className="mt-3 leading-7 text-slate-700">{t.servicesBody}</p><p className="mt-6 font-semibold text-slate-950">Ramazan Berber</p><p className="mt-1 text-sm text-slate-600">Norway</p></div><a className="mt-8 inline-flex rounded-2xl bg-[#2185DC] px-5 py-3 font-semibold text-white" href="mailto:hello@yasaflow.com">hello@yasaflow.com</a><p className="mt-5 text-sm text-slate-500">{t.response}</p></section>}
