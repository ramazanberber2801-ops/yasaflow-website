      ...
      <div className="mx-auto flex min-h-20 max-w-7xl items-center justify-between gap-4 px-5 sm:px-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <details className="relative lg:hidden">
            ...
            <nav className="absolute right-0 top-14 w-64 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl" aria-label="Mobile navigation">
              {links.map(([label, href]) => <a key={href} href={href} className="block min-h-12 rounded-xl px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-100">{label}</a>)}
            </nav>
          </details>
        </div>
      </div>
