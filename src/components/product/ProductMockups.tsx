...
export function AdministratorDashboardMockup({ locale }: { locale: Locale }) {
  const t = copy[locale];
  const cards: readonly (readonly [LucideIcon, string])[] = [[Users, t.members], [CalendarDays, t.activities], [Megaphone, t.news], [FileText, t.documents]];
  return <BrowserFrame><div className="flex min-h-72"><SideRail /><div className="min-w-0 flex-1 p-4 sm:p-5">
...
export function MobileAppMockup({ locale }: { locale: Locale }) {
  const t = copy[locale];
  const navItems: readonly (readonly [LucideIcon, string])[] = [[Home, t.home], [CalendarDays, t.calendar], [FileText, t.documents], [Users, t.profile]];
  return <div className="mx-auto w-full max-w-[230px] rounded-[2.4rem] border-[7px] border-[#0F172A] bg-white p-3 shadow-[0_24px_70px_rgba(15,23,42,.2)]">
    ...
    <nav className="mt-4 grid grid-cols-4 gap-1 border-t border-slate-100 pt-3 text-center text-[8px] text-slate-500">{navItems.map(([Icon, label], index) => <span key={label} className={index === 0 ? 'text-[#2185DC]' : ''}><Icon className="mx-auto mb-1" size={14} />{label}</span>)}</nav>
  </div>;
}
