  ...
  const startCheckout = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('create-paddle-checkout', {
        body: {
          modules: {
            pushNotifications: selectedIds.includes('push-notifications'),
            donations: selectedIds.includes('donations'),
          },
        },
      });

      if (error) throw error;
      if (!data?.checkoutUrl || typeof data.checkoutUrl !== 'string') throw new Error('Missing checkout URL');
    ...
    <main className="px-5 py-20 sm:px-8 sm:py-28"><div className="mx-auto max-w-7xl">
      ...
      <div className="mt-14 grid gap-8 lg:grid-cols-[1.05fr_.95fr]">
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-4 border-b border-slate-200 pb-7 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-[#2185DC]">{t.base}</p><div className="mt-2 flex items-end gap-2"><span className="text-5xl font-semibold">{BASE_PRICE_NOK}</span><span className="pb-1 text-sm text-slate-500">{t.perMonth}</span></div></div><button type="button" onClick={startCheckout} disabled={isStartingCheckout || !supabase} className="inline-flex min-h-12 items-center justify-center rounded-2xl bg-[#2185DC] px-5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60">{isStartingCheckout ? t.loading : t.cta}</button></div>
          {checkoutError && <p role="alert" className="mt-4 rounded-2xl bg-red-50 px-4 py-3 text-sm font-medium text-red-700">{checkoutError}</p>}
