    ...
    try {
      ...
      if (!response.ok || !payload.portal_url) {
        if (response.status === 404) {
          throw new Error('Organisasjonen har ingen aktiv fakturakonto ennå. Velg abonnement først.');
        }
        throw new Error(payload.error || 'Kunne ikke åpne faktura- og abonnementsportalen.');
      }
      window.location.assign(payload.portal_url);
    } catch (billingError) {
      setError(billingError instanceof Error ? billingError.message : 'Kunne ikke åpne faktura- og abonnementsportalen.');
    } finally {
      setBillingBusy(null);
    }
  };
